...
import MeetTheElvesPage from './components/MeetTheElvesPage';
...
function App() {
  ...
  const [currentPage, setCurrentPage] = useState<'home' | 'about' | 'events' | 'elves'>(() => {
    const savedPage = sessionStorage.getItem('currentPage');
    return (savedPage as 'home' | 'about' | 'events' | 'elves') || 'home';
  });
  ...
  const photos = [
    ...
    {
      id: 13,
      src: "/emilyelf.jpg", // NOTE: The image name is not changed, just the alt and caption
      alt: "Elf",
      caption: "Elves are always preparing"
    },
    {
      id: 14,
  ...
  if (currentPage === 'about') {
    return <AboutPage onBack={() => setCurrentPage('home')} onNavigateToEvents={() => setCurrentPage('events')} onNavigateToElves={() => setCurrentPage('elves')} />;
  }

  if (currentPage === 'events') {
    return <EventsPage onBack={() => setCurrentPage('home')} onNavigateToAbout={() => setCurrentPage('about')} onNavigateToElves={() => setCurrentPage('elves')} />;
  }

  if (currentPage === 'elves') {
    return <MeetTheElvesPage onBack={() => setCurrentPage('home')} onNavigateToAbout={() => setCurrentPage('about')} onNavigateToEvents={() => setCurrentPage('events')} />;
  }
        ...
        <div className="w-full px-4 sm:px-6 lg:px-8 xl:px-12 2xl:px-16">
          <div className="flex justify-between items-center h-16 sm:h-18 md:h-20 lg:h-24">
            ...
            <nav className="hidden sm:flex items-center space-x-1 md:space-x-2 lg:space-x-4 xl:space-x-6 2xl:space-x-8">
              ...
              <button
                onClick={() => setCurrentPage('elves')}
                className="text-xs sm:text-sm md:text-base lg:text-lg xl:text-xl text-gray-700 hover:text-red-600 transition-colors duration-200 font-medium px-1 sm:px-2 whitespace-nowrap"
              >
                Meet the Elves
              </button>
              <button
                onClick={() => window.open('https://santa-hat-society.square.site/', '_blank')}
